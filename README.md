...
这个项目依赖 Paper plugin 的 classloader / mapping 行为。
所以它 ***不是*** Bukkit插件。
切勿将他放进 spigot/spigot-fork 服务端中。
...
1. 构建 `CuteOldCombat.jar`。
2. 放入服务端 `plugins/` 目录。
3. 确保服务端启动脚本中包含以下 JVM参数:
   ```bash
   -Djdk.attach.allowAttachSelf=true
   -XX:+EnableDynamicAgentLoading
   ```
...
如果看到：
```text
[OldCombat] NMS patches were not installed. Only Bukkit-side fallbacks are active.
```
则证明版本不受支持。
